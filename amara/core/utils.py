"""
This module provides general utility for scripts written for convenience, deindentation
and wrapping.
"""
...
class ObjectStorage:
    """
    This class provides multiple object storage for general utility with `pickle`. 
    Allows for reading and writing sets of inter-related data with a customizable 
    storage format using the `amara.core.utils._Cache` storage object.
    ...
    :func:`to_pickle`
        Pickles the whole `ObjectStorage` and saves it at the `filepath` specified.
    :func:`from_pickle` 
        Loads a `ObjectStorage` stored at the filepath specified.
    """
    
    def __init__(self, max_caches: int = 5) -> None:
        """
        Instantiates an instance of `ObjectStorage`.
    ...
    def add_cache(self, **kwargs: dict[str, Any]) -> None:
        ...
        ValueError: Keyword argument "colour" does not exist in this ObjectStorage object's cache signature. Valid keyword arguments are: ['name', 'age'].
        
        Max caches functionality
        >>> storage = ObjectStorage(max_caches=2)
        >>> for i in range(5):
        ...
        # if signature set, check kwargs against signature
            ...
            for key, value in kwargs.items():
                ...
                if key not in list(self.__cache_signature.keys()):
                    raise ValueError(f'Keyword argument "{key}" does not exist in this ObjectStorage object\'s cache signature. Valid keyword arguments are: {list(self.__cache_signature.keys())}.')
    ...
    def to_pickle(self, filepath: os.PathLike) -> None:
        ...
        function. Pickles the whole `ObjectStorage` and saves it at the `filepath` 
        specified.

        Parameters
        ----------
        `filepath` : `os.PathLike`
            Filepath to save `ObjectStorage` object to.
    ...
    @classmethod
    def from_pickle(cls, filepath: os.PathLike) -> ObjectStorage:
        """
        This function is a wrapper for the built-in module pickle's `pickle.load` 
        function. Loads the `ObjectStorage` saved at the filepath and returns
        it.

        Parameters
        ----------
        `filepath` : `os.PathLike`
            Filepath where the `ObjectStorage` object is located.

        Returns
        -------
        `ObjectStorage`
            `ObjectStorage` object saved at the pickle `filepath` passed.

        Examples
        --------
        >>> storage = ObjectStorage.from_pickle('path/to/pickle.pkl')
...
class _Cache:
    """
    Helper class for `ObjectStorage`, acts as main storage object within
